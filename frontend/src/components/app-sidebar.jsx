  ...
  return (
    <aside className="hidden md:flex md:w-72 shrink-0 flex-col border-r border-sidebar-border bg-sidebar/80 backdrop-blur-xl shadow-lg shadow-black/5">
      ...
      <nav className="flex-1 space-y-1.5 px-4 overflow-y-auto">
        <p className="px-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground/70 mb-3 mt-2">
          Menu
        </p>
        {items.map(({ to, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                className={`h-5 w-5 transition-transform duration-200 ${isActive ? "" : "group-hover:scale-110 text-muted-foreground group-hover:text-primary"}`}
              />
              {label}
            </Link>
          );
    ...
    <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 grid grid-cols-5 border-t border-border/40 glass shadow-[0_-10px_40px_rgba(0,0,0,0.05)] pb-safe">
      {items.map(({ to, label, icon: Icon }) => {
        ...
        return (
          ...
          >
            <div
              className={`grid h-8 w-8 place-items-center rounded-full transition-all ${isActive ? "bg-primary/10" : ""}`}
            >
