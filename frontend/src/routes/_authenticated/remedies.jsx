...
export default function Remedies() {
  ...
  const handleSelect = (currentValue) => {
    ...
    if (isAlreadySelected) {
      setSelected((prev) =>
        prev.filter((item) => item.toLowerCase() !== currentValue.toLowerCase()),
      );
    } else {
        ...
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent className="w-full p-0" align="start">
            <Command>
              ...
              <CommandList>
                <CommandEmpty>No preset symptom found.</CommandEmpty>
                <CommandGroup>
                  {chips.map((symptom) => (
                    <CommandItem
                      key={symptom}
                    ...
                    >
                      <Check
                        className={cn(
                          "mr-2 h-4 w-4",
                          selected.some((s) => s.toLowerCase() === symptom.toLowerCase())
                            ? "opacity-100"
                            : "opacity-0",
                        )}
                      />
                  ...
                  {inputValue.trim() !== "" &&
                    !chips.some((c) => c.toLowerCase() === inputValue.trim().toLowerCase()) && (
                      <CommandItem
